...
export default function HomeIncomeModal({ navigation }) {
  ...
  // Use state to find the total monthly income - used to calculate the percentages in the legend
  ...
  // Function to query firestore - retreives income records belonging to the user in the current month for a specified category
  function setCount(category) {
    return firebase.firestore()
      ...
      .then(querySnapshot => {
        ...
        // Initialise category income count = 0
        let categoryCount = 0;

        // Add to category income amount with each returned record
  ...
  // Use effect for fetching income data
  useEffect(() => {
  ...
  // Chart configuration sets properties of monthly income pie chart
  const chartConfig = {
